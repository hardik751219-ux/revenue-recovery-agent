...
↓
[ DETECTOR ] — sliding-window failure-rate analysis per (payment_method, route),
with overlapping windows merged to avoid double-counting one incident
↓
Degradation Alert (with revenue at risk)
↓
[ ROOT-CAUSE CLASSIFIER ] — majority-vote on failure_reason_code within the alert window
↓
Diagnosis + Confidence Score
↓
[ RECOVERY POLICY / BOUND CHECKER ] — hard-coded gates, not LLM judgment
↓
ALLOW ──────────────────→ BLOCKED / ESCALATED (logged, no action taken)
↓
[ AGENT REASONING LAYER ] — advisory only, can flag caution, cannot override the bound checker
↓
[ RAZORPAY TEST-MODE API ] — real order creation
↓
Success → verified, logged, revenue counted as recovered
Failure → bounded retry (max 2) → graceful abandonment, no infinite loop
↓
